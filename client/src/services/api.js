/**
 * API service – Sellora Jewelry Store
 * Fetches FakeStore products + 84 curated mock jewelry items = 100+ total
 */

const FAKESTORE = 'https://fakestoreapi.com';

// ── Mock jewelry catalogue (84 items) ────────────────────────────────────────
// Using reliable Unsplash CDN images – no API key needed
...
/**
 * Fetch all FakeStore products and merge with curated mock jewels.
 * Returns 100+ items total.
 */
export async function fetchJewelry() {
  try {
    ...
    // Combine: mock jewels first (they're the primary catalogue), then FakeStore
    return [...MOCK_JEWELS, ...fakeItems];
  } catch {
    // If FakeStore is down, return mock jewels only – app stays functional
    return [...MOCK_JEWELS];
  }
}
 ...
 * Checks mock data first, then falls back to FakeStore API.
 */
...
/**
 * Sign in via FakeStore auth endpoint.
 */
